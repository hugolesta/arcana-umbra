...
- **Trampas de UI ya pisadas** (no repetirlas):
  ...
  - **Base64 de imágenes de entrada se trunca en tránsito por encima de ~10-15KB** (parámetros MCP tipo `image_base64`/`init_image_base64`/`color_image_base64`): el tool falla con "broken data stream" o directamente decodifica basura sin avisar. Achicar la imagen de origen (ej. `sips -Z 96 in.png --out small.png`) antes de leerla y pasarla como base64 evita el problema — no hace falta el tamaño original, el modelo igual la reinterpreta.
  - **`create_portrait_character` (`character_to_portrait`) no preserva la identidad del personaje de forma confiable**: probado contra el Viajero existente (capa violeta, capucha, carta dorada) devolvió un personaje genérico sin relación visual. Además `view="side"` sólo soporta `result_size=64` (128/160 tiran "no internal style" y sugieren bajar a 64 o cambiar a `view="low top-down"`). Para un busto/retrato que debe verse como un personaje ya existente, **usar `create_image_pixflux` con una descripción de texto rica** (colores, prendas, pose, objeto en mano) en vez de intentar derivarlo de la imagen — es el método que terminó funcionando para `retrato_viajero.png`.
  - **`create_image_pixflux` con `init_image_base64` o `color_image_base64` puede devolver basura silenciosa** en vez de fallar: con `init_image_base64` (strength 150) sobre un sprite 96×96 el resultado fue una imagen completamente transparente/vacía; con `color_image_base64` colapsó a una silueta negra plana (el color dominante del contorno se propagó a toda la imagen). Ninguno de los dos avisa del problema — hay que abrir el PNG resultante (`Read` o revisar alpha) antes de asumir que salió bien. Si el objetivo es "mismo estilo/paleta que X" más que "edición de X", preferir descripción de texto detallada sin init/color image.
  - **Los jobs asíncronos de PixelLab (`get_image`, `get_ui_asset`, `get_portrait_character`, etc.) se esperan con la tool `Monitor`** (poll loop con condición de salida), no con `Bash run_in_background` + `sleep N` + consulta manual repetida — este último funciona pero genera 8-12 idas y vueltas innecesarias por asset. Un loop tipo `until`/`while` en `Monitor` que consulta el estado del job y corta cuando `status=completed` da una sola notificación de "listo".
